...
GENESIS_PROTOCOL_VERSION=$(jq -r '.genesis_protocol_version // 67' ${BM_PARAMS})
...
gen_localnet_for_forknet() {
    ...
    $MIRROR new-test --epoch-length ${EPOCH_LENGTH} --num-validators ${NUM_CHUNK_PRODUCERS} \
        --num-seats ${NUM_SEATS} --new-chain-id localnet \
        --genesis-protocol-version ${GENESIS_PROTOCOL_VERSION} --yes
...
tweak_config_forknet() {
    ...
    # Apply custom node-specific configs to each chunk producer
    local node_index=0
...
tweak_config_forknet_node() {
    ...
    # Apply case-specific config patches if they exist
    if [ -f "${BASE_CONFIG_PATCH}" ]; then
        jq -s 'reduce .[] as $item ({}; . * $item)' \
            ${CONFIG} ${BASE_CONFIG_PATCH} >tmp.$$.json && mv tmp.$$.json ${CONFIG} || rm tmp.$$.json
    fi
    ...
    # Configure logging
    edit_log_config ${NEAR_HOME}/log_config.json
}
