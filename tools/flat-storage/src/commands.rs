...
impl FlatStorageCommand {
    ...
    fn move_flat_head_back(
        ...
        for _ in 0..blocks {
            let block_hash = chain_store.get_block_hash_by_height(height.clone())?;
            ...
            let mut prev_iter = prev_trie.iter()?;
            ...
            let mut missing_keys: HashSet<_> =
            ...
            for key in missing_keys {
                ...
                if prev_value != value {
                    prev_delta.insert(key.to_vec(), prev_value.cloned());
                }
            }
