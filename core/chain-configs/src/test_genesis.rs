...
enum ValidatorsSpec {
    DesiredRoles {
        ...
        chunk_only_validators: Vec<String>,
    },
    Raw {
...
impl TestGenesisBuilder {
    ...
    pub fn validators_desired_roles(
        ...
        self.validators = Some(ValidatorsSpec::DesiredRoles {
            block_and_chunk_producers: block_producers.iter().map(|s| s.to_string()).collect(),
            chunk_only_validators: chunk_only_producers.iter().map(|s| s.to_string()).collect(),
        });
        self
    }
    ...
    pub fn validators_raw(
        ...
        num_chunk_only_validator_seats: NumSeats,
    ) -> &mut Self {
        self.validators = Some(ValidatorsSpec::Raw {
            ...
            num_chunk_validator_seats: num_block_and_chunk_producer_seats
                + num_chunk_only_validator_seats,
        });
        self
    }
...
fn derive_validator_setup(specs: ValidatorsSpec) -> DerivedValidatorSetup {
    match specs {
        ValidatorsSpec::DesiredRoles { block_and_chunk_producers, chunk_only_validators } => {
            let num_block_and_chunk_producer_seats = block_and_chunk_producers.len() as NumSeats;
            let num_chunk_only_validator_seats = chunk_only_validators.len() as NumSeats;
            ...
            for i in 0..num_chunk_only_validator_seats as usize {
                let account_id: AccountId = chunk_only_validators[i].parse().unwrap();
                let account_info = AccountInfo {
                    public_key: create_test_signer(account_id.as_str()).public_key(),
                    account_id,
                    amount: ONE_NEAR
