...
use super::{Children, RawTrieNode, TrieRefcountDeltaMap};
...
impl Trie {
    ...
    pub fn find_state_part_boundary(
        ...
        let maybe_root_node = self.retrieve_node(&self.root)?;
        let total_size =
            maybe_root_node.as_ref().map(|(_, node)| node.memory_usage).unwrap_or_default();
        let size_start = total_size / num_parts * part_id + part_id.min(total_size % num_parts);
        let root_node = maybe_root_node.as_ref().map(|(_, node)| node);
        ...
        let mut handle_branch = |value_handle: Option<&ValueRef>,
         ...
         -> Result<bool, StorageError> {
            ...
            while let Some((index, child)) = iter.next() {
                let child = self.retrieve_node(child)?.ok_or(StorageError::UnexpectedTrieValue)?.1;
            ...
            Err(StorageError::StorageInconsistentState(format!(
                "Skipped all children {children:?} while searching for memory \
                threshold {memory_threshold} and skipped {memory_skipped}"
            )))
        };

        match &node.node {
            RawTrieNode::Leaf(key, _) => {
            ...
            RawTrieNode::BranchWithValue(value_handle, children) => {
                handle_branch(Some(value_handle), children)
            }
            RawTrieNode::BranchNoValue(children) => handle_branch(None, children),
            RawTrieNode::Extension(key, child_handle) => {
                let child =
                    self.retrieve_node(child_handle)?.ok_or(StorageError::UnexpectedTrieValue)?.1;
                let (slice, _) = NibbleSlice::from_encoded(key);
                key_nibbles.extend(slice.iter());
                *node = child;
                Ok(true)
    ...
    fn find_node_in_dfs_order(
        &self,
        root_node: Option<&RawTrieNodeWithSize>,
        memory_threshold: u64,
    ) -> Result<Vec<u8>, StorageError> {
