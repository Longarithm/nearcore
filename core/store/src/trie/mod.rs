...
#[derive(Debug, Eq, PartialEq)]
pub(crate) struct RawTrieNodeWithSize {
    node: RawTrieNode,
    memory_usage: u64,
}
...
impl Trie {
    ...
    pub fn DEBUG_chunk_cache_size(&self) {
        if let Some(storage) = self.storage.as_caching_storage() {
            ...
            #[cfg(feature = "deepsize_feature")]
            let deep_size_of = (*cache).deep_size_of();
            #[cfg(not(feature = "deepsize_feature"))]
            let deep_size_of = 0;
            tracing::debug!(target: "runtime", len = cache.len(), sum_lens = sum_lens, deep_size_of = deep_size_of);
        }
    }
}
