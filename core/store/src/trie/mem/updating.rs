...
pub(crate) trait GenericTrieUpdate<'a, GenericTrieNodePtr, GenericValueHandle> {
    ...
    /// Inserts new node into the set of updated nodes.
    ...
    for MemTrieUpdate<'a, M>
{
    ...
    fn generic_insert_node(&mut self, node: UpdatedMemTrieNodeWithSize) -> GenericUpdatedNodeId {
        let index = self.updated_nodes.len();
        self.updated_nodes.push(Some(node.node));
        index
    }
...
>: GenericTrieUpdateSquash<'a, N, V>
{
    fn calc_memory_usage_and_store(
        ...
        let new_memory_usage =
            children_memory_usage.saturating_add(new_node.memory_usage_direct()).saturating_sub(
                old_child
                    .map(|child| self.generic_get_node(child).memory_usage)
                    .unwrap_or_default(),
            );
    ...
    fn generic_insert(
                ...
                GenericUpdatedTrieNode::Branch { children, value: old_value } => {
                    if partial.is_empty() {
                        ...
                        let new_node_id = match child.take() {
                            Some(node_id) => self.generic_ensure_updated(node_id)?,
                            None => {
                                self.generic_insert_node(GenericUpdatedTrieNodeWithSize::empty())
                            }
                        };
                ...
                GenericUpdatedTrieNode::Leaf { extension, value: old_value } => {
                    ...
                    if common_prefix == existing_key.len() && common_prefix == partial.len() {
                        ...
                        let branch_node = if existing_key.is_empty() {
                            ...
                            let new_node_id =
                                self.generic_insert_node(GenericUpdatedTrieNodeWithSize {
                                    node: new_node,
                                    memory_usage,
                                });
                ...
                GenericUpdatedTrieNode::Extension { extension, child: old_child, .. } => {
                    ...
                    if common_prefix == 0 {
                        ...
                        let child = if existing_key.len() == 1 {
                            ...
                            GenericNodeOrIndex::Updated(self.generic_insert_node(inner_child))
                        };
                        ...
                        let child_id = self.generic_ensure_updated(old_child)?;
        ...
        for i in (0..path.len() - 1).rev() {
            ...
            let child_memory_usage = self.generic_get_node(*child_id).memory_usage;
            let mut node = self.generic_take_node(*node_id);
            node.memory_usage += child_memory_usage;
            self.generic_place_node(*node_id, node);
        }
    ...
    fn generic_delete(
        ...
        for node_id in path.into_iter().rev() {
            ...
            let GenericUpdatedTrieNodeWithSize { node, mut memory_usage } =
                self.generic_take_node(node_id);
            memory_usage += child_memory_usage;
            self.generic_place_node(node_id, GenericUpdatedTrieNodeWithSize { node, memory_usage });
            ...
            child_memory_usage = self.generic_get_node(node_id).memory_usage;
        }
