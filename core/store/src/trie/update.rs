    ...
    pub fn deref_value(&self) -> Result<Vec<u8>, StorageError> {
        match self {
            ...
            TrieUpdateValuePtr::HashAndSize(trie, _, hash) => {
                trie.trie_node_cache.borrow_mut().deref_mut().retrieve_raw_bytes(hash, |hash| trie.trie.storage.retrieve_raw_bytes(hash))
            },
        }
    }
}
...
impl TrieUpdate {
    ...
    pub fn get_ref(&self, key: &TrieKey) -> Result<Option<TrieUpdateValuePtr<'_>>, StorageError> {
        ...
        self.trie.get_ref(&self.root, &key, self.trie_node_cache.borrow_mut().deref_mut()).map(|option| {
            option.map(|(length, hash)| TrieUpdateValuePtr::HashAndSize(&self, length, hash))
        })
    }
