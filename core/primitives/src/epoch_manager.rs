        ...
        pub fn sample_chunk_producer(
            ...
            match &self {
                ...
                Self::V4(v4) => {
                    ...
                    let shard_id = shard_id as usize;
                    let sampler = v4.chunk_producers_sampler.get(shard_id);
                    let sampler =
                        sampler.ok_or(EpochError::new_sharding_error(shard_id, v4.epoch_height))?;
                    let sample = sampler.sample(seed);
                    let settlement = v4.chunk_producers_settlement.get(shard_id);
                    let settlement = settlement
                        .ok_or(EpochError::new_sharding_error(shard_id, v4.epoch_height))?;
                    let cp = settlement.get(sample);
                    cp.ok_or(EpochError::new_sampling_error(sample, shard_id, v4.epoch_height))
                        .copied()
                }
            }
        }
