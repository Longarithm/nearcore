...
use near_primitives::config::default_read_memory_trie_node;
...
static ALL_COSTS: &[(Cost, fn(&mut EstimatorContext) -> GasCost)] = &[
    ...
    (Cost::ReadMemoryTrieNode, read_memory_trie_node),
...
fn read_memory_trie_node(ctx: &mut EstimatorContext) -> GasCost {
    GasCost::from_gas(default_read_memory_trie_node().into(), ctx.config.metric)
}
