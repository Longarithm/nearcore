...
fn get_prev_epoch_identifier(
    chain: &Chain,
    first_block: Option<(CryptoHash, Utc)>,
) -> Option<ValidatorInfoIdentifier> {
    let epoch_first_block_info = first_block?;
    let epoch_start_block_header = chain.get_block_header(&epoch_first_block_info.0).ok()?;
...
impl ClientActorInner {
    ...
    fn get_epoch_info_view(
        ...
        tracing::debug!(target: "chain", "DEBUGDEBUG got validator info");
        let epoch_height =
            self.client.epoch_manager.get_epoch_info(&epoch_id).map(|info| info.epoch_height())?;
        tracing::debug!(target: "chain", "DEBUGDEBUG epoch_height: {:?}", epoch_height);
        Ok(EpochInfoView {
            epoch_height,
            epoch_id: epoch_id.0,
    ...
    fn get_recent_epoch_info(
        ...
        tracing::debug!(target: "chain", "DEBUGDEBUG get_recent_epoch_info epoch_id: {:?}", epoch_id);
        ...
        tracing::debug!(target: "chain", "DEBUGDEBUG before getnext: {:?}", epoch_identifier);
        ...
        tracing::debug!(target: "chain", "DEBUGDEBUG after getnext: {:?}", epoch_identifier);
        ...
        for _ in 0..DEBUG_EPOCHS_TO_FETCH {
            ...
            let first_block = epoch_view.first_block.clone();
            epochs_info.push(epoch_view);

            tracing::debug!(target: "chain", "DEBUGDEBUG fetched epoch: {:?}", current_epoch_identifier);
            ...
            tracing::debug!(target: "chain", "DEBUGDEBUG current epoch: {:?}", current_epoch_identifier);
        }
    ...
    fn get_last_blocks_info(
        ...
        tracing::debug!(target: "chain", "DEBUGDEBUG get_last_blocks_info: {:?}", query);
