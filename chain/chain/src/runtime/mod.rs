    ...
    fn query(
        ...
        match request {
            ...
            QueryRequest::ViewShardLayout {} => {
                ...
                // Get shard layout for current epoch
                let shard_layout = epoch_manager.get_shard_layout(epoch_id).map_err(|err| {
                    crate::near_chain_primitives::error::QueryError::from_epoch_error(
                        err,
                        block_height,
                        *block_hash,
                ...
                let epoch_start_height =
                    epoch_manager.get_epoch_start_height(epoch_id).map_err(|err| {
                        crate::near_chain_primitives::error::QueryError::from_epoch_error(
                            err,
                            block_height,
                            *block_hash,
                ...
                // Calculate epoch end height (if available)
                let epoch_end_height =
                    if let Ok(next_epoch_id) = epoch_manager.get_next_epoch_id(epoch_id) {
                        epoch_manager.get_epoch_start_height(&next_epoch_id).ok()
                    } else {
                        None
                    };
                ...
                let validators =
                    epoch_manager.get_epoch_block_producers_ordered(epoch_id).map_err(|err| {
                        crate::near_chain_primitives::error::QueryError::from_epoch_error(
                            err,
                            block_height,
                            *block_hash,
                ...
                // Get tracked shards information from the network
                let tracking_shards = self.get_tracking_shards_info(&shard_layout, &validators);

                // Construct the response
                let shard_layout_view = ShardLayoutView {
                    version: shard_layout.version(),
                    shard_ids: shard_layout.shard_ids().collect(),
                    boundary_accounts: shard_layout.get_boundary_accounts().cloned().collect(),
                    shards_split_map: shard_layout
                        .get_split_map()
                ...
                Ok(QueryResponse {
                    kind: QueryResponseKind::ViewShardLayout(shard_layout_view),
                    block_height,
                    block_hash: *block_hash,
                })
            }
...
// Helper method to get information about which nodes track which shards
fn get_tracking_shards_info(
    &self,
    shard_layout: &ShardLayout,
    validators: &[ValidatorStake],
) -> HashMap<ShardId, Vec<NodeInfo>> {
    let mut tracking_shards = HashMap::new();

    // Initialize the map with empty vectors for each shard
    for shard_id in shard_layout.shard_ids() {
        tracking_shards.insert(shard_id, Vec::new());
    }

    // Add validators to their respective shards
    for validator in validators {
        let account_id = validator.account_id().clone();
        let is_validator = true;

        // Get the shards this validator is assigned to
        let assigned_shards = self.get_validator_assigned_shards(&account_id, shard_layout);

        // Construct node info
        let node_info = NodeInfo {
            url: format!("http://{}.near:3030", account_id), // This is a placeholder - in reality, you'd get this from network info
            account_id: Some(account_id),
            is_validator,
            is_archival: false, // This would come from node configuration
        };

        // Add this node to each shard it tracks
        for shard_id in assigned_shards {
            if let Some(nodes) = tracking_shards.get_mut(&shard_id) {
                nodes.push(node_info.clone());
            }
        }
    }

    // Add non-validator RPC nodes (this would come from network info)
    // This is a placeholder - in reality, you'd get this from network state
    let rpc_nodes = self.get_rpc_nodes();
    for node in rpc_nodes {
        let node_info = NodeInfo {
            url: node.url.clone(),
            account_id: node.account_id.clone(),
            is_validator: false,
            is_archival: node.is_archival,
        };

        // RPC nodes typically track all shards or a specific subset
        for shard_id in node.tracked_shards.iter() {
            if let Some(nodes) = tracking_shards.get_mut(shard_id) {
                nodes.push(node_info.clone());
            }
        }
