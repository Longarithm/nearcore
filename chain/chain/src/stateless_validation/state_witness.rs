...
impl Chain {
    pub fn create_and_save_state_witness(
        &mut self,
    ...
    fn collect_state_transition_data(
        ...
        let main_transition = ChunkStateTransition {
            ...
            base_state,
            post_state_root: *self.get_chunk_extra(main_block, &shard_uid)?.state_root(),
        };
    ...
    fn collect_source_receipt_proofs(
        ...
        let prev_chunk_original_block: BlockHeader = {
            ...
            loop {
                ...
                cur_block = self.chain_store().get_block_header(cur_block.prev_hash())?;
            }
        };
        ...
        let prev_prev_chunk_block =
            self.chain_store().get_block(prev_chunk_original_block.prev_hash())?;
        ...
        let incoming_receipt_proofs = self.chain_store().get_incoming_receipts_for_shard(
            self.epoch_manager.as_ref(),
            prev_chunk_header.shard_id(),
        ...
        for receipt_proof_response in incoming_receipt_proofs {
            let from_block = self.chain_store().get_block(&receipt_proof_response.0)?;
